...
# install_github("nowickm2/BiomarkerWrappers",  host="https://github.roche.com/api/v3")
...
data_cat2 <- mutate_all(bdata[, genes], cut_core_2groups)
...
data_cat4 <- mutate_all(bdata[, genes], cut_core_quartiles)
...
biomarker_vars <- c("GeneA", "GeneA_cat2")
adjustment_vars <- "IPI"
...
```{r Complex_barplot, fig.height=5, fig.width=7}
...
data <- bdata
data$Cell_Of_Origin[data$Cell_Of_Origin == "UNCLASSIFIED"] <- NA
x_var <- "Cell_Of_Origin"
y_var <- "GeneA"
...
```{r Point_plot_strat, fig.width = 10}
...
wrapper_core_kruskal_test_col_cat(data, num_var = num_var, cat_var = cat_var)
