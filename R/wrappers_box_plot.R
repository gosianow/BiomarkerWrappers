...
wrapper_box_plot_core <- function(data, x_var, y_var, dodge_var = NULL, facet_var = NULL, color_point_var = NULL,
  ...
  if(show_total_counts || show_median){
    ...
    if(is.null(dodge_var)){
      ...
      ggpl <- ggpl +
        geom_text(data = ggdata_summ, aes(x = .data[[x_var]], y = ymin - ynudge, label = .data[["Label"]]), fontface = "plain", size = label_size, size.unit = "pt", vjust = 0.5)
      
    }else{
      
      ggpl <- ggpl +
        geom_text(data = ggdata_summ, aes(x = .data[[x_var]], y = ymin - ynudge, group = .data[[dodge_var]], label = .data[["Label"]]), fontface = "plain", size = label_size, size.unit = "pt", vjust = 0.5, position = position_dodge(preserve = "total", width = 0.9))
