...
wrapper_pearsons_test_core <- function(data, response_var, covariate_var, strata_vars = NULL, method = "pearson", variable_names = NULL, caption = NULL, force_empty_cols = FALSE, print_total = TRUE, print_non_response = TRUE, print_pvalues = TRUE){
  ...
  out <- data.frame(Covariate = variable_names[res$covariate], 
    ...
    as.data.frame(matrix(format_difference_CIs(res[, paste0(response_levels[2], "_CI95_lower")], res[, paste0(response_levels[2], "_CI95_upper")], digits = 1), ncol = 1, dimnames = list(NULL, paste0(response_levels[2], " 95% CI")))),
    check.names = FALSE, stringsAsFactors = FALSE)
  
  
  
  if(method == "pearson"){
    out$Difference = format_difference(res$difference, digits = 1, non_empty = 1)
    out$`Difference 95% CI` = format_difference_CIs(res$difference_CI95_lower, res$difference_CI95_upper, digits = 1, non_empty = 1)
